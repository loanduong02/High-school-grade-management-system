...
if(isset($_POST['add'])) {
    ...
    ?>
    <br/>
    <table style="margin-left:70px ;" border="1" cellspacing="0" cellpadding="2">
        ...
        $query="select * from hocsinh";
        $results=mysqli_query($conn, $query);
        $query1="select * from namhoc,day,giaovien,hocky, lophoc, diem,hocsinh,monhoc WHERE namhoc.NamHoc=hocky.NamHoc && giaovien.Magv=day.Magv && hocky.MaHocKy=diem.MaHocKy && lophoc.MaLopHoc=hocsinh.MaLopHoc && diem.MaHS=hocsinh.MaHS && diem.MaMonHoc=monhoc.MaMonHoc";
        $results1 =mysqli_query($conn, $query1);
        $row=mysqli_fetch_assoc($results1);
        ?>
        <form action="add_diemhs.php " method="post">
            <hr>
            <div style="text-align:center;margin-left: 400px;border: 2px solid;width:500px;" >
            <div> <b> Danh Sách Lớp: </b> <?php echo "$row[Tenlophoc]" ?></div>
                <div> <b>Môn Học: </b> <?php echo "$row[TenMonHoc]" ?></div>
                <div> <b>Học Kỳ:</b> <?php echo "$row[TenHocKy]" ?>  </div>
                <div><b>Năm Học: </b>  <?php echo "$row[NamHoc]" ?></div>
                <div> <b> Giáo Viên: </b> <?php echo "$row[Tengv]" ?></div>
            </div>
            <hr>
            <div>
